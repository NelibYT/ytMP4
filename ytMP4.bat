...
echo: & echo Veuillez patienter, chargement... & bin\yt-dlp.exe -f bestaudio %url% -o exports\tmp\audio.mp3 --ffmpeg-location bin\ffmpeg.exe > nul:
if not exist exports\tmp\audio.mp3 set wrk=1
if %wrk%==0 echo Telechargement en cours... & bin\yt-dlp.exe -f bestvideo %url% -o exports\tmp\video.mp4 --ffmpeg-location bin\ffmpeg.exe > nul:
if not exist exports\tmp\video.mp4 set wrk=1
if %wrk%==0 echo Telechargement reussi. & echo: & echo Encodage avec ffmpeg en cours... & bin\ffmpeg.exe -i exports\tmp\video.mp4 -i exports\tmp\audio.mp3 -c:v copy -c:a aac exports\%ttl%.mp4
