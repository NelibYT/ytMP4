...
ping www.youtube.com -n 1 -w 1000 >nul & if errorlevel 1 powershell -command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('ytMP4 Skin ne peut pas se connecter a internet. Verifiez votre connexion.', 'ytMP4 Skin 1.3', 'OK', [System.Windows.Forms.MessageBoxIcon]::Warning)}" & exit
...
if not "%maj:~15,-2%"=="1.3" powershell -command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Une nouvelle version de ytMP4 est disponible. Voulez-vous la telecharger?', 'ytMP4 Skin 1.3', 'YesNo', [System.Windows.Forms.MessageBoxIcon]::Information);}">"bin\settings.tmp" & set /p dld=<"bin\settings.tmp" & del "bin\settings.tmp"
...
if exist "%folder%\%ttl:~0,-5%.mp4" powershell -command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Conversion reussie :P', 'ytMP4 Skin 1.3', 'OK', [System.Windows.Forms.MessageBoxIcon]::Information)}" & explorer "%folder%"
::Message pour conversion ratée
if not exist "%folder%\%ttl:~0,-5%.mp4" powershell -command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Une erreur est survenue. Verifiez le lien ou votre connexion puis reessayez.', 'ytMP4 Skin 1.3', 'OK', [System.Windows.Forms.MessageBoxIcon]::Error)}" & exit
