...
set conv="& {Add-Type -AssemblyName Microsoft.VisualBasic; [Microsoft.VisualBasic.Interaction]::InputBox('Collez l'URL de votre video ici:', 'Convertisseur YouTube vers MP4')}" > bin\settings.tmp
powershell -Command %conv%
...
if %vc%==0 set result="& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Conversion reussie :P', 'Convertisseur YouTube vers MP4', [System.Windows.Forms.MessageBoxIcon]::Information);}" & explorer exports
if %vc%==1 set result="& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Une erreur est survenue. Verifiez l'URL ou votre connexion puis reessayez.', 'Convertisseur YouTube vers MP4', [System.Windows.Forms.MessageBoxIcon]::Warning);}"
